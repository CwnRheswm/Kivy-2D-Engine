...
def rotate_by_slope(actor, ghost, **kwargs):
   #print('rotate by slope: ', actor.impact, actor.impactLines)
    ...
    if 'target' in kwargs.keys():
        print('new target')
        impact, linesDict = get_intersection(actor, ghost, kwargs['target'])
        if impact != None:   #if it hits the new target
            actor.impact = [actor.impact, impact]
    ...
    if (prevSlope < surfSlope and selfSlope > surfSlope) or (prevSlope > surfSlope and selfSlope < surfSlope):
        ...
        while near(selfSlope, surfSlope) == False:
            ...
            ghost.apply_transform(r, post_multiply = True, anchor = actor.to_local(*actor.impact))
            ghost.pointsDict, ghost.linesDict = get_widget_bound(ghost)
            selfSlope = round(ghost.linesDict[selfM.strip('Line') + "M"], 3)'''
...
def bounce(actor, velocity, target, impact, impactLines):
    ...
    #print('bounce check: ',actor.center_x, impact)
    #if actor.center_x == impact[0]:
    #   #print('CENTER')
    if actor.center_x > impact[0]:
        #print('bounce right')
        rotDirect = 1
    elif actor.center_x < impact[0]:
        #print('bounce left')
        rotDirect = -1
    else:
        #print('CENTER')
...
def line_intersect(line1, line2):
    ...
    Returns:
        False if lines are parallel, but non-collinear
...
def get_intersection(actor, ghost, target):
    ...
    for each in ghPts.keys(): #and/or each in trPts.keys():
        ...
        midPt = Vector((((newPt[0] - actor.impact[0]) * math.cos(math.radians(actor.rotSpd/2)))
                        - ((newPt[1] - actor.impact[1]) * math.sin(math.radians(actor.rotSpd/2))))
                       (((newPt[0] - actor.impact[0]) * math.sin(math.radians(actor.rotSpd/2)))
                        + ((newPt[1] - actor.impact[1]) * math.cos(math.radians(actor.rotSpd/2)))))
        #find quadratic
        ...
        for j in targetLines.keys():
            ...
            if ghost.linesDict[str(i).strip('Line') + 'M']  == target.linesDict[str(j).strip('Line') + 'M']:
                if check_border(target.linesDict[j], ghost.linesDict[i], actor.linesDict[i]):
                   #print('Flat Slope')
                    #impact = line_intersect((((Vector(ghost.linesDict[i][0]) + Vector(ghost.linesDict[i][1]))/2),((Vector(actor.linesDict[i][0]) + Vector(actor.linesDict[i][1]))/2)),target.linesDict[j])
                    #impactLines = [i,j]
    ...
    '''iPts = ([(x,y), '#Line', '#Line'], ...)'''
        #if len(iPts) == 2:
        #    for point in iPts:
        #        ghostCord = point[0], point[0] - (ghost.velocity) #Line between impact
        #        actorimpact = point[0] - (ghost.velocity) #The position of the impact points on the actor
    '''
        impacts = {}
    ...
    '''     impacts={}
            ...
            for i,j in newImp.items():
                ...
                for j in range(0,2):
                    if point_on_lines(newImp[0], newImp[j]):
                        #on the Actor()
                    for i in range(len(newimpact)):
                        orange = line_intersect(target.linesDict(iPts[i][2]), [newimpact[0], Vector(newimpact[0]) + Vector(ghost.velocity)])
                        if point_on_lines(orange, target.linesDict(iPts[i][2])):
                            impacts{newimpact} = orange
                        else:
                            pass
            ...
            if iPts[0][1] == iPts[1][1]:
                ...
                #point on the target between the two impacts
                redPt = 
    ...
    for y in range(yRng):
        for x in range(xRng):
            ...
            if ghost.deltaRot != 0:
                if actor.impact != None:
                    rotPoint = actor.impact
                else:
    ...
    #print('Velo: ', velo)
   #print('striking @ ', ghost.x, ghost.center_x, impact, impactLines)
    ghost.velocity = velo
    ghost.pos = pos
    ghost.rotation = rot
...
def get_ghosts(actors):
    ...
    futures = {}
    for actor in actors:
        if actor.impact != None or actor.velocity != Vector(0,0):
            ghost = scatter_ghost(actor)
            ghost.pos = (Vector(*ghost.pos) + Vector(actor.velocity))   #get new ghost position
            #print('get_ghosts: impact: ', actor.impact, actor.rotDirect)
            if actor.impact == None:
               #print('is pushed')
            ...
            #print('rotating ghosts: ', actor.impact)
    ...
    for actor in futures.keys():
        ...
        if ghost not in bbox_collisions.keys():     #ghost did not collide
            ...
            if actor.impactLines != []: #if there IS an impactLine, i.e. has already collided with something
               #print('rotation check: ', actor.impact)
               ...
               #print('free ghost  ', ghost.pos)
                ...
                if impactLines != []:
                   #print('to bounce')
                    velocity, rotDir, impactLines = bounce(actor, ghost.velocity, target, impact, impactLines) #set new velocity, rotation
                    #print('!= [] :', impactLines)
        #print('actor to impact: ',actor.impact)
        move[actor] = [pos, rotation, velocity, rotDir, impact, impactLines]
...
def move_actors(move):
    ...
    for actor in move.keys():
       #print('move: ',move[actor])
        [pos, rotation, velocity, rotDir, impact, impactLines] = move[actor]
        actor.pos = pos
