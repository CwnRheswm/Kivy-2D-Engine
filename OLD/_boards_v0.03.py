...
class BlackBoard(Scatter):
    ...
    def on_touch_move(self, touch):
        ...
        if self.grabbed != None:
            ...
            #            if check_inside([point], target.ghostLines) == True or check_inside(target.pointsDict.values(), self.grabbed.ghostLines) == True:
            #                #self.grabbed.do_translation = False
            #                self.grabbed.pos = self.grab_pos
            #                #print(point, self.grabbed.pos)
            
            #while self.grabbed in collisions:
                #if self.grabbed in collisions:
                #n = remove_vector(collisions[self.grabbed].keys(),  self.grabbed.cog)
                #self.grabbed.pos = Vector(self.grabbed.pos) + Vector(n)
                #collisions = get_collisions(self.actors + [self.grabbed], self.scenery)
            self.grab_pos = self.grabbed.pos
            #if check_inside([touch.pos], self.grabbed.ghostLines) == False:
            #    self.grabbed = None
            #    self.touch_timer = 0.0
        ...
        if self.grabbed != None:
            ...
            if self.grabbed in collisions.keys():
                ...
                if point[0] > 100:
                    ...
                    target = self.grabbed
                    point = point[0]
                n = Vector(-(2*touch.dx), -(2*touch.dy))#remove_vector(target.pointsDict[point],  self.grabbed.cog)
                #print('1',self.grabbed.pos,n,touch.dx,touch.dy,touch.dz)
                self.grabbed.pos = Vector(self.grabbed.pos) + Vector(n)
                #print('2',self.grabbed.pos,n)
                #self.grabbed.ghostPoints = get_widget_bounds(self.grabbed)
                #collisions = get_collisions(self.actors + [self.grabbed], self.scenery)
                #print(collisions)
            self.grab_pos = self.grabbed.pos
