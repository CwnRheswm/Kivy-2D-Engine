...
def scatter_ghost(target):
    ...
    ghost.pos = Vector(ghost.pos) + Vector(ghost.velocity)
...
def check_inside(points, target):
    lines = []
    for point in target.pointsDict.keys():
        i = target.pointsDict[point]
        j = point + 1
        if j == len(target.pointsDict):
            j = 0
        lines.append([i, target.pointsDict[j]])
...
def get_collisions(ghosts, scenery):
    collisions = {}
    ...
    for ghost in ghosts:
        for target in ghosts + scenery:
            if ghost != target: #check to make sure you aren't running the target against itself
                
                for point in ghost.pointsDict.keys():  #run each point in the actor against each target's lines
                    if target.collide_point(ghost.pointsDict[point]):
                        if check_inside([ghost.pointsDict[point]], target):
                            #if type(actor) == Fruit:
                                #print('collision, actor ghostPoints: '+str(actor.ghostPoints[point]))
                            if ghost in collisions.keys():
                                if point in collisions[ghost].keys():
                                    collisions[ghost][point].append(target)
                                else:
                                    collisions[ghost][point] = [target]
                            else:
                                collisions[ghost] = {}
                                collisions[ghost][point] = [target]
                #!!!!!!
...
def get_future_pos(actors, scenery):
    ...
    for actor in actors:
        ...
        ghost.pointsDict = get_widget_bounds(ghost)
        ghosts.append(ghost)
        
    collisions = get_collisions(ghosts, scenery) #collisions{ actors {points: [targets]} }
    impacts = {}
    #!!!
    for actor in actors:
        impact = {}
